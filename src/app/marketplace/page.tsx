...
import { Search, Filter, Book, Heart, ShoppingBag, Loader2, Star, Trash2, Wallet } from "lucide-react";
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="space-y-8 mb-12">
          ...
          <div className="flex flex-wrap gap-6 items-center bg-white p-6 rounded-[2rem] shadow-xl shadow-black/5 border border-black/5">
            ...
            <div className="flex items-center gap-4">
              <span className="text-xs font-black uppercase tracking-widest text-foreground/30">Lọc theo Xu (Coin):</span>
              <div className="flex gap-2">
                {[null, 10, 50, 100].map(val => (
                  <button
                    key={String(val)}
                    onClick={() => setMaxCoinFilter(val)}
                    className={cn(
                      "px-4 py-2 rounded-xl text-xs font-bold transition-all",
                      maxCoinFilter === val ? "bg-primary text-white" : "bg-black/5 text-foreground/40 hover:bg-black/10"
                    )}
                  >
                    {val === null ? "Tất cả" : `<= ${val} Xu`}
                  </button>
                ))}
              </div>
            </div>
                  ...
                  <div className="absolute top-3 right-3 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 z-10">
                    <button 
                      ...
                      className={cn(
                        ...
                        wishlistIds.includes(book.id) ? "bg-red-500 text-white" : "bg-white hover:bg-primary hover:text-white dark:text-black"
                      )}
                    >
